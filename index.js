...
function getType(target) {
  if (Array.isArray(target)) return 'array';else if (target && (target.buffer || target instanceof ArrayBuffer)) return 'arraybuffer';else if (target == null) return 'null';
...
function stringifyArray(array, lengthLimit, seen) {
  if (isNumberArray(array)) {
    ...
    if (array.length <= lengthLimit) return `[${res}]`;else return `[${res}]|length ${array.length}, tpye array|`;
  } else {
    let res = "[";
    for (let i of array) {
...
function stringifyArrayBuffer(arraybuffer, lengthLimit) {
  ...
  if (arraybuffer.length <= lengthLimit) return `[${res}]`;else return `[${res}]|length ${arraybuffer.length}, tpye arraybuffer|`;
}
...
class StackTrace {
  ...
  proxy(proxyObject, name, debug = undefined) {
    return proxy(proxyObject, name, (name, mode, target, property, value) => {
      ...
      let content;
      let text;
      if (this.details) content = this.stringify(target);else content = this.stringify(value);
